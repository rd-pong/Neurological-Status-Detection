...
void c_CompactClassificationECOC_pre(const double obj_ClassNames[4], const
  ...
  [6], const double obj_CodingMatrix[24], const double Xin[28], double *labels,
  double negloss[4])
{
  int idx;
  double M[24];
  ...
  for (idx = 0; idx < 6; idx++) {
    for (partialTrueCount = 0; partialTrueCount < 28; partialTrueCount++) {
    ...
    classnum = 0.0;
    for (partialTrueCount = 0; partialTrueCount < 28; partialTrueCount++) {
      classnum += Xnorm[partialTrueCount] / obj_BinaryLearners[idx].Scale *
        obj_BinaryLearners[idx].Beta[partialTrueCount];
    }

    pscore[idx] = classnum + obj_BinaryLearners[idx].Bias;
  }
